...
StarField::StarField(Game* owner, Vec2 const& startPos)
...
void StarField::UpdateAlphaColor(float deltaSeconds)
{
	...
	if (!m_game->m_playerTank->m_isDead)
	{
		m_velocity = m_counterPlayerVelocityScale * (m_game->m_playerTank->m_velocity);
		m_velocity *= -1.f;
	}
